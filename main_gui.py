...
def run_analysis_tool():
    ...
    with st.sidebar:
        ...
        kepco_tariff = st.selectbox(
            ...
            index=["HV_C_III", "HV_C_I", "HV_C_II"].index(st.session_state.config['kepco_tariff']),
        ...
        with col1:
            start_date = st.date_input(
                "Start Date",
                value=pd.to_datetime(st.session_state.config['start_date']),
                help="First date to include in the analysis period"
            )
        with col2:
            end_date = st.date_input(
                "End Date",
                value=pd.to_datetime(st.session_state.config['end_date']),
                help=f"Last date to include in the analysis period (max {app_settings['max_analysis_days']} days)"
            )
        # Soft guard in UI for over-long ranges
        try:
            num_days = (pd.to_datetime(end_date) - pd.to_datetime(start_date)).days + 1
            if num_days > int(app_settings['max_analysis_days']):
                st.warning(f"Selected range is {num_days} days. Maximum allowed is {int(app_settings['max_analysis_days'])} days.")
        except Exception:
            pass
        ...
        load_capacity_mw = st.number_input(
            "Load Capacity (MW)",
            value=float(st.session_state.config['load_capacity_mw']),
            min_value=0.1,
            step=100.0,
        ...
        ppa_price = st.number_input(
            "PPA Price (KRW/kWh)",
            value=float(st.session_state.config['ppa_price']),
            min_value=0.0,
            step=1.0,
        ...
        ppa_mintake = st.slider(
            ...
            min_value=0,
            max_value=100,
            value=int(st.session_state.config['ppa_mintake'] * 100),
            step=1,
        ...
        ppa_resell = st.checkbox(
            "Allow Reselling",
            value=bool(st.session_state.config['ppa_resell']),
        ...
        ppa_resellrate = st.slider(
            ...
            min_value=0,
            max_value=100,
            value=int(st.session_state.config['ppa_resellrate'] * 100),
            step=1,
        ...
        with col1:
            ppa_range_start = st.number_input(
                "Start (%)",
                value=int(st.session_state.config['ppa_range_start']),
                min_value=0,
                step=10,
        ...
        with col2:
            ppa_range_end = st.number_input(
                "End (%)",
                value=int(st.session_state.config['ppa_range_end']),
                min_value=0,
                step=10,
        ...
        with col3:
            ppa_range_step = st.number_input(
                "Step (%)",
                value=int(st.session_state.config['ppa_range_step']),
                min_value=1,
                step=1,
        ...
        carbon_price = st.number_input(
            ...
            value=float(st.session_state.config['carbon_price']),
            min_value=0.0,
            step=1000.0,
        ...
        ess_include = st.checkbox(
            ...
            value=bool(st.session_state.config['ess_include']),
        ...
        ess_capacity = st.slider(
            ...
            min_value=0,
            max_value=200,
            value=int(st.session_state.config['ess_capacity'] * 100),
            step=10,
        ...
        ess_price = st.slider(
            ...
            min_value=0,
            max_value=100,
            value=int(st.session_state.config['ess_price'] * 100),
            step=5,
            disabled=not ess_include,
        ...
        export_long_format = st.checkbox(
            "Export Long Format",
            value=bool(st.session_state.config['export_long_format']),
        ...
        verbose = st.checkbox(
            "Verbose Output",
            value=bool(st.session_state.config['verbose']),
...
def display_results():
    ...
    has_carbon_price = st.session_state.config['carbon_price'] > 0
...
def export_results():
    ...
    try:
        ...
        st.download_button(
            ...
            data=excel_bytes,
            file_name=st.session_state.config['output_file'],
